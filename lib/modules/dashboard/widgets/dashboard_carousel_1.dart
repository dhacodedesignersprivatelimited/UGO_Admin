...
import '../view_model/old_dashboard_view_model.dart';
...
class _DashboardCarousel1State extends State<DashboardCarousel1>
  ...
  List<double> _lastWeekSeries(List<double> thisWeek) {
    if (thisWeek.isEmpty) return [];
    final rnd = math.Random(42);
    return thisWeek
        .map((e) => (e * (0.68 + rnd.nextDouble() * 0.22)).clamp(0.0, 1e12))
        .toList();
  }
  ...
  Widget _earningsPage(DashboardPageModel m) {
    ...
    final lastWeek = _lastWeekSeries(thisWeek);
    ...
    final spotsLast = List.generate(
      ...
      (i) => FlSpot(
        i.toDouble(),
        i < lastWeek.length ? lastWeek[i] * _anim.value : 0,
      ),
    );

    final allY = [...spotsThis.map((e) => e.y), ...spotsLast.map((e) => e.y)];
